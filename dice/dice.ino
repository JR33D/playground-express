...
#define R_COLOR              0          // Color RED value
#define G_COLOR              0          // Color GREEN value
#define B_COLOR            255          // Color BLUE value
...
void loop() {
  ...
  if (newRoll) {
    if (millis() - rollStartTime > 1500) rolling = false;
  }
  ...
  if (rolling) {
    ...
    for (int p=0; p<rollNumber; p++) {
      CircuitPlayground.setPixelColor(dicePixels[rollNumber-1][p],  uint8_t(R_COLOR), 
                                                                    uint8_t(G_COLOR), 
                                                                    uint8_t(B_COLOR));
    }    
    delay(20);    
